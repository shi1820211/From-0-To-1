...
def parse_excel_content(rows):
    ...
    for i in range(1, len(rows)):
        ...
        contaminated_specimens = str(row[4]).strip() if len(row) > 4 else ""
...
def main():
    ...
    if len(headers) >= 5:
        ...
        header_categories = [
            ...
            extract_category_from_header(headers[4])  # 第五列表头
        ]
    else:
        # 如果表头不完整，使用默认分类
        header_categories = ["致病菌", "定植菌", "正常菌", "污染菌"]

    print(f"提取的表头分类: {header_categories}")
    ...
    for i, data in enumerate(bacteria_data):
        ...
        # 处理第五列（索引4）- 新增的污染菌列
        if len(headers) > 4:
            contaminated_samples = extract_sample_types_and_counts(data["污染标本"])
            for sample in contaminated_samples:
                sample["expected_category"] = header_categories[3]  # 使用第五列表头的分类
                sample_types_and_counts.append(sample)
