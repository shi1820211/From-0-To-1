...
def read_excel_text(file_path):
    ...
    for sheet_name in xls.sheet_names:
        ...
        for start_row in range(0, total_rows, MAX_ROWS_PER_CHUNK):
            ...
            # 添加前后文叠加
            context_start = max(0, start_row - 1)  # 包含前一个块的最后一行
            context_end = min(total_rows, end_row + 1)  # 包含后一个块的第一行
            chunk = df.iloc[context_start:context_end]
            lines = []
